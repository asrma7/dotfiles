...
from gi.repository import Gtk, Gdk, GLib, Gtk4LayerShell, Pango, PangoCairo
import calendar
import datetime
import json
import math
...
THEME_FILE = os.path.expanduser('~/.config/waybar-ycal/matugen/matugen-waybar-ycal.toml')
...
def _video_uri_from_text(*values):
    urls = []
    for value in values:
        if not value:
            continue
        urls.extend(_clean_url(match.group(0)) for match in URL_RE.finditer(value))
    ...
    for url in urls:
        lower_url = url.lower()
        if any(marker in lower_url for marker in join_markers):
            return url

    for url in urls:
        lower_url = url.lower()
        if '/ics?' not in lower_url and not lower_url.endswith('.ics'):
            return url

    return None


def _event_video_uri(item):
    conference = item.get('conferenceData') or {}
    for entry in conference.get('entryPoints', []):
        uri = entry.get('uri') if entry.get('entryPointType') == 'video' else None
        if uri:
            return uri
    ...
    return _video_uri_from_text(item.get('location'), item.get('description'))


def _event_entry(item, label, start_dt=None, end_dt=None):
    entry = {
        'type': 'event',
        'title': item.get('summary', '(no title)'),
        'label': label,
    }
    video_uri = _event_video_uri(item)
    if video_uri:
        entry['videoUri'] = video_uri
    if start_dt is not None:
        entry['start'] = start_dt.isoformat()
    if end_dt is not None:
        entry['end'] = end_dt.isoformat()
    return entry


def _is_task(ev):
    return isinstance(ev, dict) and ev.get('type') == 'task'


def _is_event(ev):
    return isinstance(ev, dict) and ev.get('type') == 'event'
            ...
            for item in result.get('items', []):
                ...
                if 'dateTime' in start:
                    ...
                    events_by_date.setdefault(dt.date().isoformat(), []).append(
                        _event_entry(item, label, dt, dt_end)
                    )
                else:
...
class CalendarPopup(Gtk.ApplicationWindow):
    ...
    def _build_grid(self):
        ...
        for i in range(total_days):
            ...
            has_events = any(_is_event(e) for e in day_events)
            has_tasks = any(_is_task(e) and not e.get('done') for e in day_events)
    ...
    def _make_event_row(self, ev):
        ...
        if _is_task(ev):
            ...
            label = ev.get('label') or ev.get('title', '(no title)')
            video_uri = ev.get('videoUri')
            dot = Gtk.Label(label="•")
            ...
            row.append(name)
            if video_uri:
                row.add_css_class('event-link-row')

                link_icon = Gtk.Label(label="↗")
                link_icon.add_css_class('event-link-icon')
                link_icon.set_tooltip_text(video_uri)
                row.append(link_icon)

                left_click = Gtk.GestureClick()
                left_click.set_button(1)
                left_click.connect('pressed', self._on_event_open, video_uri)
                row.add_controller(left_click)

                right_click = Gtk.GestureClick()
                right_click.set_button(3)
                right_click.connect('pressed', self._on_event_copy, video_uri)
                row.add_controller(right_click)

        return row

    def _on_event_open(self, *_args):
        link = _args[-1]
        subprocess.Popen(['xdg-open', link])
        self._hide()

    def _on_event_copy(self, *_args):
        link = _args[-1]
    ...
    def _update_day_panel(self, date):
        ...
        day_events = sorted(
            ...
            key=lambda e: 0 if (_is_task(e) and not e.get('done')) else (1 if _is_task(e) else 2)
        )
        if day_events:
